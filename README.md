...
You can run the pipeline at four levels, from plain Python up to a scheduler. Snakemake, covered in the next section, can then drive the plain-Python, Apptainer and SLURM levels for you. All commands run from the project root.
...
config.yaml         gene list, species ID and STRING request parameters
data/               stage 1 output (edges.csv), regenerated by the pipeline
outputs/            stage 2-3 outputs (hub_genes.csv, network.png)
